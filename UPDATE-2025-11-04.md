...
- 句尾點規則（重點優化）：
  - 含 CJK 字元的行（即使夾雜英數字）且「恰好一個」`.` 作為行尾，移除該尾點。
  - 若行尾為連續兩個以上的 `.`（例如 `..`、`...`），完全保留，不進行刪除（避免 `...` 被誤減為 `..` 或空）。
  - 若尾點前的有效字元為結尾標點（`?`, `!`, `？`, `！`, `…`），視為多餘點，僅移除最後一個 `.`，保留原結尾標點。
  - ASCII 標頭樣式行（僅含英數/空白/連字號、且含冒號，如 `Story: ...`、`Lang: ...`）若以 `.` 結尾，移除該尾點。
  - 其他正規化：合併多重換行、修整換行前後空白、移除僅包含單一 `.` 的行。
- 讀取邊界：段落文字迴圈邏輯改為 `index <= maxIndex`，避免最後一個字元被漏讀（如 `Lang: TRD` 的尾字 `D`）。
- 保留省略號：「…」不再被清除。
- 強化 W16 換行後同步：遇到 `0xFE` 時，改以掃描法尋找下一段文字開頭（`0x00`+ASCII 或 Big5 前/後導位元），避免因固定位移造成位元組錯位而出現亂碼；若掃描失敗才回退到既有位移邏輯。
- 清理控制碼：在正規化階段移除 `\x00` 與其他不可見控制字元（保留換行/Tab），並移除可見的占位字串 `<0x00>`。

案例與參考
- 修正 DFA573929-TRD.fpc：
  - `於是"Zillow驚奇屋"誕生了.` → `於是"Zillow驚奇屋"誕生了`
  - `這是1927年T4車款駕駛室.` → `這是1927年T4車款駕駛室`
- 修正 DFA267904-TRD.pac（多點保留）：
  - `卡內薩達是牛肉,波優薩多..` 保留 `..`
  - `歐洽達類似米...` 保留 `...`
...
- Preserve ellipsis: the character `…` is no longer stripped.
 - Stronger W16 resync after newlines: when encountering `0xFE`, scan ahead for the next text start (`0x00`+ASCII or a valid Big5 lead/trail pair) to avoid byte misalignment and garbled output; fall back to prior offset rule if scanning fails.
 - Control cleanup: during normalization, strip `\x00` and other non-printable control chars (keeping newline/tab), and remove visible placeholder token `<0x00>` if present.
